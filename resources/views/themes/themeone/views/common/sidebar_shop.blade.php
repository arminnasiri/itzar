 ...
 <div id="accordion"  class="filters" role="tablist">
 	...
 	<form enctype="multipart/form-data" name="filters" method="get">
        ...
        <div class="card">
            ...
            @endif
            <div class="block">
                <div class="mr10 select2shopsidebar">
            @if(count($result['manufacturers']))
                <select style="width:90%;" class="select2 " name="manufacturers_id[]" multiple="multiple" id="sidebar_shop">
                    @foreach ($result['manufacturers'] as $manufacturer)
                        <option value="{{ $manufacturer->manufacturers_id }}">{{ $manufacturer->manufacturers_name }}</option>
                    @endforeach
                </select>
            @endif
                </div>
