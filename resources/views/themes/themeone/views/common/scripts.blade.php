...
<!-- zoom -->
<script src="{!! asset('public/js/zoom/Drift.min.js') !!}"></script>
<!-- easy zoom -->
<!-- <script src="{!! asset('public/js/easyzoom/easyzoom.js') !!}"></script> -->
<!--- one signal-->
<script src="{!! asset('public/js/multiselect/select2.min.js') !!} " ></script>
...
function getQuantity(){
	...
	d={'attrid':tem};
	 ...
	 var fd= formData.split('&');
	 var xz=0;
	 for(var i=0;i<fd.length;i++){
		if(fd[i].indexOf('products_id') > -1){
			let temp = fd[i].split("=");
			d[temp[0]]= temp[1];
			xz++;
		}
		if(fd[i].indexOf('customer_id') > -1){
			let temp = fd[i].split("=");
			d[temp[0]]= temp[1];
			xz++;
		}
	...
	jQuery.ajax({
		...
		success: function (res) {
			// setTimeout(function(){
			// 	if(res.is_teammate){
			// 		jQuery('#pay_make').text(res.pay_make);	
			// 		jQuery('#pay').text('res.pay');	
			// 	}else{
			// 		jQuery('#is_teammate').hide();
			// 	}
			// 	jQuery('.total_price').text(res.price);
			// },200);
			...
			if(res.is_teammate){
				...
				jQuery('.total_price').text(res.price);
				jQuery('#products_price').val(res.price);
...
// my js file
function dzoom(){
	var demoTrigger = document.querySelector('#dzoom');
	var paneContainer = document.querySelector('#pzoom');
	if(demoTrigger!=undefined || paneContainer!=undefined ){
...
function imageZoom(imgID, resultID) {
  ...
  img.addEventListener("touchmove", moveLens);
	let pzoom =document.getElementById('pzoom');
  ...
  function moveLens(e) {
    ...
    if (x > img.width - lens.offsetWidth) {
        x = img.width - lens.offsetWidth;pzoom.style.display="none";
    }
    if (x < 0) {
        x = 0;pzoom.style.display="none";
    }
    if (y > img.height - lens.offsetHeight) {
        y = img.height - lens.offsetHeight;pzoom.style.display="none";
    }
    if (y < 0) {
        y = 0;pzoom.style.display="none";
    }
