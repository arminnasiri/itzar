                	...
                	<div class="detail-area">
                        ...
                        @endif
                                ...
                                <div id="product-slider" class="carousel slide">
									...
									@endphp
                                        <span products_id='{{$result['detail']['product_data'][0]->products_id}}' class="icon-slider fa @if($result['detail']['product_data'][0]->isLiked==1) fa-heart @else fa-heart-o @endif is_liked">
                                    ...
                                    <div class="carousel-inner">
                                        ...
                                        <div class="active item carousel-item " data-slide-number="0">
                                                <img id='dzoom' class="img-thumbnail" data-zoom="{{asset('').$result['detail']['product_data'][0]->products_image }}" src="{{asset('').$result['detail']['product_data'][0]->products_image }}" alt="img-fluid">
                                        </div>

                                        @foreach( $result['detail']['product_data'][0]->images as $key=>$images )
                                            <div class="item carousel-item" data-slide-number="{{++$key}}">
                                                <?php   echo "<span class='discount-tag'>".(int)$discount_percentage."%</span>"; ?>
                            ...
                            @endif 
                                <div id="pzoom" style="height:100%;width:100%;">
                                </div>    
                                <div class="product-summary" >
                                    ...
                                    <div class="text-right">
                                        <span>  
                                            &nbsp; @lang('website.brand'): <a href="">{{$result['detail']['product_data'][0]->manufacturers_name}}</a>
                                        </span> 
                                        @if(!empty($result['category_name']) and !empty($result['sub_category_name']))
                                           
                                           <span>@lang('website.Category') : </span> <a href="{{ URL::to('/shop?category='.$result['sub_category_slug'])}}" class="category">{{$result['sub_category_name']}}</a>
                                        @elseif(!empty($result['category_name']) and empty($result['sub_category_name']))
                                           
                                            <span>@lang('website.Category') : </span> <a href="{{ URL::to('/shop?category='.$result['category_slug'])}}" class="category">{{$result['category_name']}}</a>
                                        @endif
                                    </div>    
                                    <hr>           

                                    <!-- <div class="text-right">
                                        <div class="orders">@lang('website.count')&nbsp;@lang('website.Order(s)')&nbsp;:{{$result['detail']['product_data'][0]->products_ordered}}</div>                                                                                
                                     ...
                                     <div class="product-price">
                                        ...
                                        <!-- <span class="price @if(!empty($result['detail']['product_data'][0]->discount_price) or !empty($result['detail']['product_data'][0]->flash_price)) line-through @else change_price @endif" >
                                            {{$web_setting[19]->value}}{{number_format($result['detail']['product_data'][0]->products_price+0)}}
                                        </span> -->

                                        @if($result['detail']['product_data'][0]->products_min_order>0)
                                        	<div class="min-order-tag">
                                       		&nbsp; @lang('website.Min Order Limit:') {{ $result['detail']['product_data'][0]->products_min_order }}
                                            </div>
                                        @endif
                                        <div class="weight" >
                                            &nbsp; @lang('website.weight'): {{ $result['detail']['product_data'][0]->products_weight }}
                                        </div>
                                        <hr>                                 
                                    </div>
                                    ...
                                    <form name="attributes" id="add-Product-form" method="post" >
                                        <br>
                                        <input type="text" name="customer_id" hidden value="<?php if(isset(auth()->guard('customer')->user()->customers_id)){echo  auth()->guard('customer')->user()->customers_id;}else{echo 0;} ?>" >
                                        <input type="hidden" name="products_id" value="{{$result['detail']['product_data'][0]->products_id}}">
                                        
                                        <input type="hidden" name="products_price" id="products_price" value="@if(!empty($result['detail']['product_data'][0]->flash_price)) {{$result['detail']['product_data'][0]->flash_price+0}} @elseif(!empty($result['detail']['product_data'][0]->discount_price)){{$result['detail']['product_data'][0]->discount_price+0}}@else{{number_format($result['detail']['product_data'][0]->products_price+0)}}@endif">
                                            ...
                                            <div class="form-row">                                             
                                                ...
                                                <div class="form-group col-sm-4">							
                                                    ...
                                                    <div class="col-sm-12">								
                                                        ...
                                                        <!-- <select name="{{ $attributes_data['option']['id'] }}" onChange="getPrice()" class="currentstock form-control attributeid_<?=$index++?>" attributeid = "{{ $attributes_data['option']['id'] }}"> -->
                                        ...
                                        <div class="form-inline product-box cont">

                                            <div class="cont"  id="is_teammate">
                                                <div>
                                                    <span>@lang('website.wage_make')&nbsp; : </span>
                                                    <span id="pay_make"></span>
                                                </div>    
                                                <div>    
                                                    <span>@lang('website.wage')&nbsp; : </span>
                                                    <span id="pay"></span>
                                                </div>    
                                            </div>
                                            
                                            <div class="form-group Qty cont--item2" @if(!empty($result['detail']['product_data'][0]->flash_start_date) and $result['detail']['product_data'][0]->server_time < $result['detail']['product_data'][0]->flash_start_date ) style="display: none" @endif>
                                            ...
                                            <div class="price-box cont--item3">
                                             ...
                                             @if(!empty($result['detail']['product_data'][0]->flash_start_date) and $result['detail']['product_data'][0]->server_time < $result['detail']['product_data'][0]->flash_start_date )
                                              ...
                                              @else
                                               @if($result['detail']['product_data'][0]->products_type == 0)
                                                    ...
                                                    @else 
                                                        <button class="btn btn-secondary add-to-Cart" type="button" products_id="{{$result['detail']['product_data'][0]->products_id}}"><i class="fa fa-cart-plus fa-3x"></i>@lang('website.Add to Cart')</button>
                                                    @endif                                                    
                                                @else
                                    ...
                                    <article>
                                        ...
                                        <?php
                                            ...
                                            if($after_date>=$current_date){
                                                print '<span class="new-tag">'.__('website.New').'</span>';
                                            }
                                        ...
                                        <div class="price text-center">
                                            @if(!empty($products->discount_price))
                                                {{$products->discount_price+0}}{{$web_setting[19]->value}}
                                                <span>{{number_format($products->products_price+0)}}{{$web_setting[19]->value}}</span>
                                            @else
                                              {{number_format($products->products_price+0)}}{{$web_setting[19]->value}}
                                            @endif
                                        </div> 
