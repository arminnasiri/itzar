...
class AdminProductsController extends Controller
{
	...
	public function products(Request $request){
		...
		$data = DB::table('products')
			...
			if(isset($_REQUEST['categories_id']) and !empty($_REQUEST['categories_id'])){
				...
				$products = $data->orderBy('products.products_date_added', 'DESC')->paginate(40);
	...
	public function addproduct(Request $request){
		...
		$result['customer_group'] = DB::table('customer_group')
            ->join('customer_group_description', 'customer_group.customers_group_id', '=', 'customer_group_description.customers_group_id')
            ->select('customer_group.*', 'customer_group_description.customers_group_title')
            ->get();
	...
	public function addnewproduct(Request $request){
		...
		$products_id = DB::table('products')->insertGetId([
                    ...
                    'products_delivery_time' =>	  $request->products_delivery_time,
                    'products_appro_weight'	 =>	  $request->product_appro_weight,
                    'product_supply_for'	 =>	  $request->supply,
                    'barcode'	             =>	  $request->products_barcode,
...
//                'id_category' => $item,
//                'id_sub_category' => $request->sub_category[$this->i],
//                'id_product' => $products_id,
//                'id_part' => $request->part[$this->i],
//            ]);
//            $this->i++;
//        }
//		$this->i=0;
            foreach ($request->customer_pay as $item) {
                DB::table('product_customer_group_pay_cent')->insert([
                    'pay' => $item,
                    'pay_make' => $request->customer_pay_make[$this->i],
                    'id_product' => $products_id,
                    'id_customer_group' => $request->customers_group_id[$this->i],
                ]);
                $this->i++;
            }
		$slug_flag = false;
    ...
    //showstock 
    public function sub_inventory(Request $request){
	    if(isset($request->ref_id))
        {
            $stock = DB::table('inventory')
                ->join('inventory_detail', 'inventory.inventory_ref_id', '=', 'inventory_detail.inventory_ref_id')
                ->where('inventory.reference_code',$request->ref_id)
                ->join('products_attributes', 'products_attributes.products_id', '=', 'inventory_detail.products_id')
                ->join('products_description', 'products_attributes.products_id', '=', 'products_description.products_id')
                ...
                ->select('inventory.stock','products_options_descriptions.options_name','products_options_values_descriptions.options_values_name','products_description.products_name')
                ->get();
            if(count($stock)==0)
	        ...
	        {
                return response()->json(['msg'=>'ref_id '],404);
            }


    }
    //substock
    public function sub_stock(Request $request){
        if(isset($request->ref_id )&&isset($request->sub_number))
        {
            $stock = DB::table('inventory')
                ->where('reference_code',$request->ref_id)
                ->sum('stock');
            if($stock==0)
                return response()->json(['msg'=>'Inventory is zero & not found'],400);
            if($stock<$request->sub_number)
                return response()->json(['msg'=>'The inventory is less than the input'],401);
            $stock=$stock-($request->sub_number);
            $stock = DB::table('inventory')
                ->where('reference_code',$request->ref_id)
                ->update(['stock'=>$stock]);
            if($stock)
            ...
            return response()->json(['msg'=>'ref_id & sub_number '],402);
    ...
    public function addnewstock(Request $request){
        if(session('products_create')==0 or session('products_update')==0){
            ...
            $inventory_ref_id=DB::table('inventory')->where('reference_code',$request->reference_code)->get();
            if(count($inventory_ref_id)>0)
            {
                return redirect()->back()->withErrors([Lang::get("labels.barcoderror")]);
            }
            else {
	...
	public function addnewproductattribute(Request $request){
		...
		$products_attributes_id = DB::table('products_attributes')->insertGetId([
                ...
                'product_attr_mode'	    =>	 $request->product_attr_mode
				]);
	...
	public function updateproduct(Request $request){
		...
		foreach($languages as $languages_data){			
            ...
            DB::table('product_customer_group_pay_cent')->where([
                'id_product'  	=>   $products_id,
            ])->delete();
            foreach ($request->customer_pay as $item) {
                DB::table('product_customer_group_pay_cent')->insert([
	...
	public function allCollection(){
		$title = array('pageTitle' => Lang::get("labels.AddProduct"));
		return view("admin.pcollection.pcollection" , $title);	
	}
	// add collection
	public function addCollection(){
		$title = array('pageTitle' => Lang::get("labels.AddProduct"));
		return view("admin.pcollection.pcollectionadd" , $title);	
	}
	// edit collection
	public function editCollection(){
		$title = array('pageTitle' => Lang::get("labels.AddProduct"));
		return view("admin.pcollection.pcollectionedit" , $title);	
	}
