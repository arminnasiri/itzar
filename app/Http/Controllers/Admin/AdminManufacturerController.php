        ...
        if(session('manufacturer_view')==0){
            ...
            $manufacturers = DB::table('manufacturers')
                ...
                ->select('manufacturers.manufacturers_id as id',
                    ...
                    'mobile',
                    'email')
                ->where('manufacturers_info.languages_id', '1')->orderBy('name', 'asc')->paginate(10);
    ...
    public function addnewmanufacturer(Request $request){
        if(session('manufacturer_create')==0){
            ...
            if($validator->fails()){
                ...
                $name=DB::table('manufacturers')->where('manufacturers_name',$request->name)->get();
                if(count($name)>0)
                    return redirect()->back()->withErrors([Lang::get("labels.manuFacturernameMessage")]);
