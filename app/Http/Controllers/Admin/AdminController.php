...
class AdminController extends Controller
{
			...
			if(auth()->guard('admin')->attempt($adminInfo)) {
				...
				if(auth()->guard('admin')->user()->adminType != '1'){
					...
					if(!empty($categories_role) and count($categories_role)>0){
						...
						$manufacturers_id=$categories_role[0]->manufacturers_ids;
					}else{
						$categories_id = '';
                        $manufacturers_id='';
					}
                ...
                session(['manufacturers_id' => $manufacturers_id]);
	...
	public function categoriesroles(Request $request){
		if(session('manage_admins_role')==0){
			...
			foreach($categories_role as $categories){				
                ...
                $man_array = explode(',',$categories->manufacturers_ids);
                $manufacturers = DB::table('manufacturers')->whereIn('manufacturers_id', $man_array)->get();
				...
				$data[$mi++]->manufacturers=$manufacturers;
			}
			$result['data'] = $data;
	...
	public function addCategoriesRoles(Request $request){
		if(session('manage_admins_role')==0){
			...
			$result['manufacturers']= DB::table('manufacturers')
                ...
                ->select('manufacturers.manufacturers_id as id',
                    ...
                    'mobile',
                    'email')
                ->where('manufacturers_info.languages_id', '1')->orderBy('name', 'asc')->get();
	...
	public function addNewCategoriesRoles(Request $request){
		if(session('manage_admins_role')==0){
			...
			if(count($exist)>0){			
                ...
                $manufacturer = array();
                foreach($request->manufacturers as $category){
				...
				$roles = DB::table('categories_role')->insert([
							'categories_ids'	=>	$categories,
                             'manufacturers_ids'	=>	$manufacturer,
