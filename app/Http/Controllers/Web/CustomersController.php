...
class CustomersController extends DataController
{
    ...
    public function processLogin(Request $request){
        ...
        $customerInfo = array("customers_telephone" => $request->phone, "password" => $request->password);
...
//		return redirect()->intended('/');
    ...
    public function processPassword(Request $request){
        ...
        $phone   		  =   $request->phone;
        //check email exist
        $existUser = DB::table('customers')->where('customers_telephone', $phone)->get();

        if(count($existUser)>0){
            DB::table('customers')->where('customers_telephone', $phone)->update([
                'password'	=>	Hash::make($password)
            ]);
...
//            $alertSetting = $myVar->forgotPasswordAlert($existUser);
            $result= Smsirlaravel::ultraFastSend(['VerificationCode'=>$password],7442 ,$phone);
            if($result['IsSuccessful'])
            {
                return redirect('login')->with('success', Lang::get("رمز ورود به شماره موبایل شما ارسال شد"));
            }
            else{
                return redirect('forgotPassword')->with('error', Lang::get("website.something is wrong"));
            }


        }else{
            return redirect('forgotPassword')->with('error', Lang::get("شماره موبایل وجود ندارد"));
        }
    ...
    public function recoverPassword(){
        ...
        return view("recoverPassword", $title);
    }
    ...
    public function signupProcess(Request $request){
        ...
        $customer_group_id =$customer_group_id->customers_group_id;
//		//validation start
        $validator = Validator::make(
            array(
                'firstName' => $request->firstName,
        ...
        if($validator->fails()){
            ...
            $user_phone = DB::table('customers')->where('customers_telephone', $phone)->get();
            if(count($user_phone)>0)
                return redirect('/signup')->withInput($request->input())->with('error', Lang::get("شماره موبایل شما قبلا در سایت ثبت شده"));
          $password=$this->createRandomPassword();
            ...
            if(DB::table('customers')->insert($data)){
                  $result= Smsirlaravel::ultraFastSend(['VerificationCode'=>$password],7439 ,$phone);

                  if($result['IsSuccessful'])
                  {
                      return redirect('login')->with('success', "ثبت نام شما با موفقیت انجام شد برای ورود از رمز ارسال شده به شمار موبایل اقدام نمایید");
                  }
                  else{
                      return redirect('/signup')->with('error', Lang::get("خطا در ارسال پیام کوتاه لطفا با پشتیبانی تماس بگیرید "));
                  }
                }else{
