...
//
			...
			$products  = $categories->skip($skip)->take(intval($take))->get();

			foreach ($products as $product)
            {
                $product->products_price=$this->get_price( $product->products_id,auth()->guard('customer')->id());
            }
			$result = array();
			$result2 = array();
    ...
    public function get_price($products_id,$customer_id=0,$attrid=null)
    {
        if($attrid==null) {
            $attrid = DB::table('products_attributes')->where('products_id', $products_id)->where('is_default', 1)->select('products_attributes_id')->get();
            $aa = array();
            foreach ($attrid as $at)
                $aa[] = $at->products_attributes_id;
            $attrid = $aa;
        }
        if($customer_id !=null) {
            $customer_group_id = DB::table('customers')->where('customers_id', $customer_id)->select('customers_group_id')->first();
            $customer_pay = DB::table('product_customer_group_pay_cent')->where('id_customer_group', $customer_group_id->customers_group_id)->where('id_product', $products_id)->first();
            $customer_group = DB::table('customer_group')->where('customers_group_id', $customer_group_id->customers_group_id)->first();
            $product = DB::table('products')->where('products_id', $products_id)->first();
            $g = $product->products_weight;
            $price = PriceBoard::find(2);
            if($customer_pay==null)
                $k=$o=1;
            else {
                $k = $customer_pay->pay / 100 + 1;
                $o = $customer_pay->pay_make;
            }
            $m = $customer_group->tax_customer_group / 100 + 1;
            $s = $customer_group->profit_customer_group / 100 + 1;
            $atrr_products = DB::table('products_attributes')->where('products_id', $products_id)->whereIn('products_attributes_id', $attrid)->get();
            ...
            $a= round(((($g * $k * $s * $m) * $price->price)  + $o),0,PHP_ROUND_HALF_UP) ;
            return round($a,-3);
        }
        else{
            $customer_group_id = DB::table('customer_group')->where('is_teammate','0')->select('customers_group_id')->first();
            $customer_pay = DB::table('product_customer_group_pay_cent')->where('id_customer_group', $customer_group_id->customers_group_id)->where('id_product', $products_id)->first();
            $customer_group = DB::table('customer_group')->where('is_teammate','0')->first();
            $product = DB::table('products')->where('products_id', $products_id)->first();
            $g = $product->products_weight;
            $price = PriceBoard::find(2);
            if($customer_pay==null)
