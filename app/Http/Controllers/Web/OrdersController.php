...
//						'updatedAt'=>$result->transaction->updatedAt->date,
		...
		foreach($order as $data){
            ...
            $id_group=DB::table('customers')->where('customers_id',session('customers_id'))->select('customers.customers_group_id')->get();
            $is_teammate= DB::table('customer_group')->where('customers_group_id',$id_group)->pluck('is_teammate');
              if($is_teammate)
                  dd($is_teammate);
                  //TODO send sms
