...
class ProductsController extends DataController
{
	...
	public function shop(Request $request){
		...
		//liked products
        $result['manufacturers']=DB::table('manufacturers')->get();
	...
	public function getquantity(Request $request){
	    ...
	    if($request->customer_id !=0) {
            $customer_group_id = DB::table('customers')->where('customers_id', $request->customer_id)->select('customers_group_id')->first();
            $customer_pay = DB::table('product_customer_group_pay_cent')->where('id_customer_group', $customer_group_id->customers_group_id)->where('id_product', $request->products_id)->first();
            $customer_group = DB::table('customer_group')->where('customers_group_id', $customer_group_id->customers_group_id)->first();
            $product = DB::table('products')->where('products_id', $request->products_id)->first();
            ...
            if(!is_null($request->attrid)) {
                $atrr_products = DB::table('products_attributes')->where('products_id', $request->products_id)->whereIn('products_attributes_id', $request->attrid)->get();
                if (!is_null($atrr_products)) {
            ...
            $data['attributes'] = $request->attrid;
			$result=$this->productQuantity($data);
            $result['price'] = round(((($g * $k * $s * $m) * $price->price) + $no_t + $o),0,PHP_ROUND_HALF_UP) ;
            $result['price']=round($result['price'],-3);
            $result['price']=$this->enToFa($result['price']);
            $result['is_teammate']=$customer_group->is_teammate;
            $result['pay']=$o;
			$result['pay_make']=$k;
			$result['weight'] = $g;
